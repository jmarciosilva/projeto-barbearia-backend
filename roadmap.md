...
| 2026-07-09 | Claude | Aprovado | Usuario testou de novo e reportou que o pagamento da assinatura de uma cliente (Fernanda, plano "Corte de Cabelo Feminino") continuava fora da receita do mes mesmo apos a correcao do fiado — pediu explicitamente que o card mostre "todo registro de pagamentos ocorrido no mes corrente", nao so avulso. Causa: a correcao anterior tinha mantido um filtro `whereNull('client_subscription_id')`, excluindo de proposito qualquer pagamento vinculado a assinatura (decisao original errada, pensando que "Recorrente do mês" ja cobria isso — mas esse card e uma projecao pelo preco do plano das assinaturas ativas, nunca reflete pagamento de verdade). Filtro removido dos dois lados da soma (pagamentos quitados sem recibo e recibos parciais) — agora conta qualquer `Payment`/`PaymentReceipt` confirmado no mes, avulso ou de assinatura. `PaymentController::index()` ganhou `subscription.plan` no eager-load (faltava, so `me()` carregava) pra a listagem do dono conseguir mostrar o nome do plano no extrato do app. 1 teste novo reproduzindo o cenario exato (assinatura + pagamento com `client_subscription_id` + `mark-paid`, conferindo que entra na soma) — 108/108 testes de backend passando | Nenhuma pendencia conhecida |
| 2026-07-09 | Claude | Aprovado | Testando o Salao da Joyce, a dona lancou o pagamento da assinatura da Fernanda duas vezes por engano (debito e credito) e nao havia forma de corrigir — usuario pediu editar/excluir lancamento direto na tela "Receita do mês" do app. Backend nao tinha nenhum endpoint de edicao/exclusao de `Payment` (so `store`/`markPaid`/`receive`); discutido em modo de planejamento antes de codificar, decisao de escopo: so lancamento "pago numa tacada so, sem recibo" fica editavel/excluivel nesta rodada (recibo parcial de fiado levanta pergunta nova sobre reverter o pagamento pai, fora do problema relatado). `PaymentController` ganhou `update()` (valor/metodo/observacao, nunca status/paid_at — isso continua exclusivo de markPaid/receive; metodo `fiado` bloqueado aqui, ja que edicao nao e uma nova confirmacao) e `destroy()` (apaga o pagamento — recibos cascateiam pela FK ja existente — e recalcula `ClientSubscription.payment_status`: so volta pra "pending" se nao sobrar nenhum outro pagamento `paid` da mesma assinatura, resolvendo exatamente o caso da Joyce, que apaga um dos dois duplicados e a assinatura continua "paga" porque o outro continua la). Rota via `apiResource('payments', ...)->only([..., 'update', 'destroy'])`, sem linha de rota nova. Bug pego durante os testes: o guard "nao deixa o valor cair abaixo do que ja foi recebido" comparava contra o accessor `received_cents`, que pra um pagamento confirmado numa tacada so (sem `PaymentReceipt`) devolve o proprio `amount_cents` de volta — bloqueava qualquer correcao de valor; corrigido pra comparar contra a soma real de `payment.receipts()`, ignorando esse caso auto-referente. 5 testes novos (editar valor/metodo; validacao de valor abaixo do recebido; excluir um duplicado mantendo assinatura "paid"; excluir o unico pagamento revertendo pra "pending"; excluir avulso; 404 pra tenant errado) — 113/113 testes de backend passando | Nenhuma pendencia conhecida |
